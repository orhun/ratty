...
impl Grid {
    ...
    fn set_row_count(&mut self, rows: u16) {
        ...
        while self.rows.len() < target {
            if let Some(row) = self.scrollback.pop_back() {
                ...
                self.rows.push(crate::row::Row::new(self.size.cols));
            }
        }
    }
    ...
    fn reflow_columns(&mut self, cols: u16) {
        ...
        let mut out: Vec<crate::row::Row> = Vec::with_capacity(all.len());
        ...
        while start < all.len() {
            ...
            let mut line_rows: Vec<Vec<crate::Cell>> = vec![Vec::with_capacity(width)];
            ...
            for (k, mut row_cells) in line_rows.into_iter().enumerate() {
                ...
                let mut row = crate::row::Row::from_cells(row_cells, k + 1 < count);
                if has_placeholder {
        ...
        while out.len() > screen_rows
            ...
            && out.last().is_some_and(crate::row::Row::is_blank)
        {
            out.pop();
        }
        let total = out.len();
        ...
        while screen.len() < screen_rows {
            screen.push(crate::row::Row::new(cols));
        }
        for row in out {
