...
/// covers the grid.
///
/// ratty-vt addition; see [`Row::has_kitty_placeholder`](crate::Row::has_kitty_placeholder).
...
pub struct Screen {
    ...
    last_print: Option<crate::grid::Pos>,
}
...
impl Screen {
    ...
    pub fn display_cursor_position(&self) -> (u16, u16) {
        ...
        if self
            .grid()
            .drawing_cell(crate::grid::Pos { row: pos.row, col })
    ...
    #[must_use]
    pub fn visible_row(&self, row: u16) -> Option<&crate::Row> {
    ...
    ///
    /// ratty-vt addition.
    pub fn visible_rows(&self) -> impl Iterator<Item = &crate::Row> {
    ...
    pub(crate) fn sgr(&mut self, params: &vte::Params, mut unhandled: impl FnMut(&mut Self)) {
        ...
        loop {
            match next_param!() {
                ...
                // ratty-vt: blink.
                [5] => self.attrs.set_blink(crate::Blink::Slow),
                [6] => self.attrs.set_blink(crate::Blink::Rapid),
                ...
                // ratty-vt: blink.
                [25] => self.attrs.set_blink(crate::Blink::None),
...
mod ratty_keyboard_tests {
    ...
    fn other_private_sgr_resources_stay_unhandled() {
        #[derive(Default)]
        struct Count(usize);
        impl crate::Callbacks for Count {
            fn unhandled_csi(
                &mut self,
                _: &mut crate::Screen,
                _: Option<u8>,
