...
//! # Fork
//!
//! This crate is Ratty's fork of [`vte`](https://crates.io/crates/vte) 0.15.0.
//! It keeps the parser and [`Perform`] API, drops the optional `ansi` module,
//! and fixes UTF-8 handling at buffer boundaries so that dispatch does not
...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    fn advance_partial_utf8<P: Perform>(&mut self, performer: &mut P, bytes: &[u8]) -> usize {
        ...
        match str::from_utf8(&self.partial_utf8[..self.partial_utf8_len]) {
            ...
            Ok(parsed) => {
                ...
                Self::ground_dispatch_char(performer, c);
            ...
            Err(err) => {
                ...
                // utf8 character into `partial_utf8`. Dispatch the completed first
                // character and consume only its bytes, so the caller re-parses
                // everything after it. Charging for all valid bytes would skip
                // characters that were copied but never dispatched.
                if valid_bytes > 0 {
                    let c = unsafe {
    ...
    /// Handle ground dispatch of a single character, executing C0 and C1 controls.
    #[inline]
    fn ground_dispatch_char<P: Perform>(performer: &mut P, c: char) {
        match c {
...
mod tests {
    ...
    fn dispatched_in_chunks(input: &[u8], chunk: usize) -> Vec<Sequence> {
        let mut dispatcher = Dispatcher::default();
        let mut parser = Parser::new();
    ...
    #[test]
    fn split_utf8_does_not_swallow_following_bytes() {
        // Upstream issue alacritty/vte#159: completing a partial scalar charged
        // the caller for every validated byte, dropping the character after it.
        let input = "B\u{301}C\u{2500}".as_bytes();
        let expected = dispatched_in_chunks(input, input.len());
        assert_eq!(
            expected,
            vec![
                Sequence::Print('B'),
                Sequence::Print('\u{301}'),
                Sequence::Print('C'),
                Sequence::Print('\u{2500}'),
            ]
        );
        for at in 1..input.len() {
            ...
            parser.advance(&mut dispatcher, &input[at..]);
            assert_eq!(dispatcher.dispatched, expected, "split at {at}");
        }
    }

    #[test]
    fn split_utf8_before_next_partial_scalar_keeps_ascii() {
        // [C3] + [A9, 'x', C3, A9] previously lost the 'x'.
        let input = "\u{e9}x\u{e9}".as_bytes();
        let expected = dispatched_in_chunks(input, input.len());
        assert_eq!(
            expected,
            vec![
        ...
        for chunk in 1..input.len() {
            assert_eq!(
                dispatched_in_chunks(input, chunk),
                expected,
                "chunk {chunk}"
            );
        }
    ...
    #[test]
    fn split_utf8_c1_control_executes_like_whole_input() {
        // Upstream issue alacritty/vte#156: a UTF-8 encoded C1 control was
        // printed when its encoding was split across `advance` calls.
        for control in 0x80..=0x9f_u8 {
            let mut buf = [0; 4];
            let encoded = char::from(control)
                .encode_utf8(&mut buf)
                .as_bytes()
                .to_vec();
            let whole = dispatched_in_chunks(&encoded, encoded.len());
            assert_eq!(whole, vec![Sequence::Execute(control)]);
            assert_eq!(
                dispatched_in_chunks(&encoded, 1),
                whole,
                "control {control:#x}"
            );
        }
    ...
    #[test]
    fn chunking_never_changes_dispatch_for_mixed_text() {
        let input = "a\u{e9}\u{80}b\u{1F469}\u{200D}\u{1F4BB}\x1b[31mc\u{2500}\n".as_bytes();
        let expected = dispatched_in_chunks(input, input.len());
        assert!(expected.contains(&Sequence::Execute(0x80)));
        assert!(expected.contains(&Sequence::Print('c')));
        for chunk in 1..input.len() {
            assert_eq!(
                dispatched_in_chunks(input, chunk),
                expected,
    ...
    #[test]
    fn invalid_partial_scalar_does_not_consume_terminating_byte() {
        for (input, expected) in [
            (
                &b"\xc3x"[..],
                vec![Sequence::Print('\u{fffd}'), Sequence::Print('x')],
            ),
            (
                &b"\xe2\x94\x1b[m"[..],
                vec![
            ...
            let expected_whole = dispatched_in_chunks(input, input.len());
            assert_eq!(expected_whole, expected, "whole {input:?}");
            assert_eq!(
                dispatched_in_chunks(input, 1),
                expected,
                "byte-wise {input:?}"
            );
        }
