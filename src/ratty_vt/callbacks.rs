...
pub trait Callbacks {
    ...
    /// (typically with `^G`).
    fn audible_bell(&mut self, _: &mut crate::ratty_vt::Screen) {}
    /// This callback is called when the terminal requests a visual bell
    /// (typically with `\eg`).
    fn visual_bell(&mut self, _: &mut crate::ratty_vt::Screen) {}
    /// This callback is called when the terminal requests a resize
    /// (typically with `\e[8;<rows>;<cols>t`).
    fn resize(&mut self, _: &mut crate::ratty_vt::Screen, _request: (u16, u16)) {}
    /// This callback is called when the terminal requests the window title
    /// to be set (typically with `\e]1;<icon_name>\a`)
    fn set_window_icon_name(&mut self, _: &mut crate::ratty_vt::Screen, _icon_name: &[u8]) {}
    /// This callback is called when the terminal requests the window title
    /// to be set (typically with `\e]2;<title>\a`)
    fn set_window_title(&mut self, _: &mut crate::ratty_vt::Screen, _title: &[u8]) {}
    ...
    /// (`\e]`) which is otherwise not implemented.
    fn unhandled_osc(&mut self, _: &mut crate::ratty_vt::Screen, _params: &[&[u8]]) {}
}
