...
where
    ...
    T: std::ops::Add<Output = T> + std::ops::Rem<Output = T> + std::ops::Sub<Output = T>,
{
...
fn contents_formatted_reproduces_state_structured(input: TerminalInput) -> bool {
...
fn qc_structured_long() {
    let mut qc = quickcheck::QuickCheck::new()
        .tests(1_000_000)
        .max_tests(1_000_000);
    qc.quickcheck(contents_formatted_reproduces_state_structured as fn(TerminalInput) -> bool);
}
...
fn qc_random_long() {
    ...
    qc.quickcheck(contents_formatted_reproduces_state_random as fn(Vec<u8>) -> bool);
}
