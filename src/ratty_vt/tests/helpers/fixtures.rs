// Vendored from doy/vt100-rust v0.16.2 tests/helpers; see ../../README.md.
    ...
    pub fn from_screen(screen: &vt100::Screen) -> Self {
        ...
        for row in 0..rows {
            for col in 0..cols {
                ...
                if cell != empty_cell {
                    cells.insert(format!("{row},{col}"), FixtureCell::from_cell(cell));
                }
            }
        }
        Self {
...
fn deserialize_color<'a, D>(deserializer: D) -> std::result::Result<vt100::Color, D::Error>
where
...
fn serialize_color<S>(color: &vt100::Color, serializer: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
    let s = match color {
...
fn load_input(name: &str, i: usize) -> Option<Vec<u8>> {
    let mut file = std::fs::File::open(format!(
        "{}/src/ratty_vt/tests/data/fixtures/{name}/{i}.typescript",
...
fn load_screen(name: &str, i: usize) -> Option<FixtureScreen> {
    let mut file = std::fs::File::open(format!(
        "{}/src/ratty_vt/tests/data/fixtures/{name}/{i}.json",
        env!("CARGO_MANIFEST_DIR")
    ))
    .ok()?;
