...
/// A logical key understood by Ratty's terminal input encoder.
#[derive(Clone, Debug, Deserialize, Eq, JsonSchema, PartialEq, Serialize)]
#[serde(tag = "kind", content = "value", rename_all = "snake_case")]
pub enum TerminalKey {
    /// Printable UTF-8 text generated by one key press.
    Text(#[schemars(length(min = 1, max = 65536))] String),
    /// Enter/Return.
    Enter,
    /// Escape.
    Escape,
    ...
    /// Delete.
    Delete,
    /// Function key number from 1 through 24.
    Function(#[schemars(range(min = 1, max = 24))] u8),
}

/// One normalized key press shared by local winit and remote control input.
#[derive(Clone, Debug, Deserialize, Eq, JsonSchema, PartialEq, Serialize)]
            ...
            match action {
                ...
                | BindingAction::ScrollDown => {
                        ...
                        && mouse_mode != MouseProtocolMode::None
                    {
                        ...
                        for _ in 0..amount {
                            params.runtime.write_human_input(&encode_mouse_wheel(
                                cell,
                ...
                BindingAction::Paste => {
                    if let Some(text) = params.clipboard.paste() {
                        ...
                        params
                            .runtime
                            .write_human_input(&encode_paste(&text, bracketed));
                    } else {
...
fn translate_key(key_code: KeyCode, ctx: KeyTranslationContext<'_>) -> Vec<u8> {
    let Some(key) = normalize_winit_key(key_code, &ctx) else {
        return Vec::new();
    };
    encode_normalized_key_with_modes(
        &key,
        ctx.application_cursor,
        ctx.kitty_keyboard_flags,
...
fn normalize_winit_key(
    key_code: KeyCode,
    ctx: &KeyTranslationContext<'_>,
) -> Option<NormalizedKey> {
    let mut modifiers = TerminalKeyModifiers {
        control: ctx.ctrl_pressed,
        alt: ctx.alt_pressed,
        shift: ctx.shift_pressed,
    };

    if ctx.alt_gr_pressed
        && let Some(text) = printable_text(ctx.text, ctx.logical_key)
    {
        return Some(NormalizedKey {
            key: TerminalKey::Text(text.to_owned()),
            modifiers: TerminalKeyModifiers::default(),
        });
    }

    let navigation = NavigationKey::from_key_code(key_code)
        .or_else(|| NavigationKey::from_logical_key(ctx.logical_key))
        .map(|key| match key {
            NavigationKey::ArrowUp => TerminalKey::ArrowUp,
            NavigationKey::ArrowDown => TerminalKey::ArrowDown,
            NavigationKey::ArrowRight => TerminalKey::ArrowRight,
            NavigationKey::ArrowLeft => TerminalKey::ArrowLeft,
            NavigationKey::Home => TerminalKey::Home,
            NavigationKey::End => TerminalKey::End,
            NavigationKey::PageUp => TerminalKey::PageUp,
            NavigationKey::PageDown => TerminalKey::PageDown,
            NavigationKey::Insert => TerminalKey::Insert,
            NavigationKey::Delete => TerminalKey::Delete,
        });

    let key = navigation.or_else(|| match key_code {
        KeyCode::Enter | KeyCode::NumpadEnter => Some(TerminalKey::Enter),
        KeyCode::Escape => Some(TerminalKey::Escape),
        KeyCode::Tab if ctx.shift_pressed => {
            modifiers.shift = false;
            Some(TerminalKey::BackTab)
        }
        KeyCode::Tab => Some(TerminalKey::Tab),
        KeyCode::Backspace => Some(TerminalKey::Backspace),
        KeyCode::Space => Some(TerminalKey::Text(" ".into())),
        KeyCode::F1 => Some(TerminalKey::Function(1)),
        KeyCode::F2 => Some(TerminalKey::Function(2)),
        KeyCode::F3 => Some(TerminalKey::Function(3)),
        KeyCode::F4 => Some(TerminalKey::Function(4)),
        KeyCode::F5 => Some(TerminalKey::Function(5)),
        KeyCode::F6 => Some(TerminalKey::Function(6)),
        KeyCode::F7 => Some(TerminalKey::Function(7)),
        KeyCode::F8 => Some(TerminalKey::Function(8)),
        KeyCode::F9 => Some(TerminalKey::Function(9)),
        KeyCode::F10 => Some(TerminalKey::Function(10)),
        KeyCode::F11 => Some(TerminalKey::Function(11)),
        KeyCode::F12 => Some(TerminalKey::Function(12)),
        KeyCode::F13 => Some(TerminalKey::Function(13)),
        KeyCode::F14 => Some(TerminalKey::Function(14)),
        KeyCode::F15 => Some(TerminalKey::Function(15)),
        KeyCode::F16 => Some(TerminalKey::Function(16)),
        KeyCode::F17 => Some(TerminalKey::Function(17)),
        KeyCode::F18 => Some(TerminalKey::Function(18)),
        KeyCode::F19 => Some(TerminalKey::Function(19)),
        KeyCode::F20 => Some(TerminalKey::Function(20)),
        KeyCode::F21 => Some(TerminalKey::Function(21)),
        KeyCode::F22 => Some(TerminalKey::Function(22)),
        KeyCode::F23 => Some(TerminalKey::Function(23)),
        KeyCode::F24 => Some(TerminalKey::Function(24)),
        _ => printable_text(ctx.text, ctx.logical_key)
            .map(|text| TerminalKey::Text(text.to_owned()))
            .or_else(|| control_text_for_keycode(key_code).map(TerminalKey::Text)),
    })?;

    Some(NormalizedKey { key, modifiers })
}

fn encode_normalized_key_with_modes(
    key: &NormalizedKey,
    application_cursor: bool,
    kitty_keyboard_flags: u8,
    modify_other_keys: Option<u8>,
    ...
    if let TerminalKey::Text(text) = &key.key {
        if text.is_empty() || text.chars().any(char::is_control) {
            return Err("printable key text must be non-empty and contain no controls".into());
        }
        if modifiers.control
            && let Some(byte) = control_text_byte(text)
        {
    ...
    let special_codepoint = match key.key {
    ...
    if modifier_code != 1
        && let Some(codepoint) = special_codepoint
    {
        if kitty_disambiguate {
            return Ok(format!("\x1b[{codepoint};{modifier_code}u").into_bytes());
        }
        if modify_other_keys.is_some() {
            return Ok(format!("\x1b[27;{modifier_code};{codepoint}~").into_bytes());
        }
    }

    let navigation = match key.key {
        TerminalKey::ArrowUp => Some(NavigationKey::ArrowUp),
        TerminalKey::ArrowDown => Some(NavigationKey::ArrowDown),
        TerminalKey::ArrowRight => Some(NavigationKey::ArrowRight),
        TerminalKey::ArrowLeft => Some(NavigationKey::ArrowLeft),
        TerminalKey::Home => Some(NavigationKey::Home),
        TerminalKey::End => Some(NavigationKey::End),
        TerminalKey::PageUp => Some(NavigationKey::PageUp),
        TerminalKey::PageDown => Some(NavigationKey::PageDown),
        TerminalKey::Insert => Some(NavigationKey::Insert),
        TerminalKey::Delete => Some(NavigationKey::Delete),
        _ => None,
    };
    if let Some(navigation) = navigation {
        return Ok(navigation.encode(
            modifiers.control,
            modifiers.alt,
            modifiers.shift,
            application_cursor,
    ...
    if let TerminalKey::Function(number) = key.key {
        return encode_function_key(number, modifiers);
    }
    ...
    match key.key {
        TerminalKey::Enter => bytes.push(b'\r'),
        TerminalKey::Escape => bytes.push(0x1b),
        TerminalKey::Tab => {
            if modifiers.shift {
        ...
        TerminalKey::BackTab => bytes.extend_from_slice(b"\x1b[Z"),
        TerminalKey::Backspace => bytes.push(0x7f),
        TerminalKey::Text(_)
        | TerminalKey::ArrowUp
        ...
        | TerminalKey::Insert
        | TerminalKey::Delete
        | TerminalKey::Function(_) => unreachable!("handled above"),
    }
    Ok(bytes)
}

fn encode_function_key(number: u8, mut modifiers: TerminalKeyModifiers) -> Result<Vec<u8>, String> {
    if !(1..=24).contains(&number) {
        return Err("function key number must be between 1 and 24".into());
    }
    let number = if number > 12 {
    ...
    let modifier_code =
        1 + modifiers.shift as u8 + (modifiers.alt as u8 * 2) + (modifiers.control as u8 * 4);
    if number <= 4 {
        let suffix = [b'P', b'Q', b'R', b'S'][usize::from(number - 1)] as char;
        return Ok(if modifier_code == 1 {
            format!("\x1bO{suffix}").into_bytes()
        } else {
            format!("\x1b[1;{modifier_code}{suffix}").into_bytes()
        });
    }
    let code = [15, 17, 18, 19, 20, 21, 23, 24][usize::from(number - 5)];
    Ok(if modifier_code == 1 {
...
fn control_text_byte(text: &str) -> Option<u8> {
...
fn control_text_for_keycode(key: KeyCode) -> Option<String> {
    ctrl_keycode_byte(key).and_then(|byte| match byte {
        0 => Some("@".into()),
        1..=26 => Some(char::from(b'a' + byte - 1).to_string()),
        27 => Some("[".into()),
        28 => Some("\\".into()),
        29 => Some("]".into()),
        30 => Some("^".into()),
...
mod tests {
    ...
    #[test]
    fn local_and_remote_normalized_navigation_match() {
        let local = translate_key(
            KeyCode::ArrowUp,
            KeyTranslationContext {
        ...
        let remote = encode_normalized_key_with_modes(
            &NormalizedKey {
        ...
        .expect("normalized arrow key must encode");
        assert_eq!(local, remote);
    }

    #[test]
    fn normalized_keys_honor_application_cursor_kitty_and_modify_other_keys() {
        let arrow = NormalizedKey {
            key: TerminalKey::ArrowUp,
            modifiers: default(),
        };
        assert_eq!(
            encode_normalized_key_with_modes(&arrow, true, 0, None)
                .expect("application cursor arrow must encode"),
            b"\x1bOA"
        );

        let ctrl_enter = NormalizedKey {
        ...
        assert_eq!(
            encode_normalized_key_with_modes(&ctrl_enter, false, 1, None)
                .expect("Kitty modified Enter must encode"),
            b"\x1b[13;5u"
        );
        assert_eq!(
            encode_normalized_key_with_modes(&ctrl_enter, false, 0, Some(2))
                .expect("modifyOtherKeys Enter must encode"),
            b"\x1b[27;5;13~"
        );
    }
    ...
    fn agent_text_preserves_utf8_exactly_and_brackets_only_when_active() {
        ...
        assert_eq!(encode_agent_text(text, true, parser.screen()), expected);
        assert_eq!(
            encode_agent_text(text, false, parser.screen()),
            text.as_bytes()
        );
    }

    #[test]
    fn normalized_function_keys_cover_f1_through_f24() {
        for number in 1..=24 {
            assert!(
                !encode_normalized_key_with_modes(
                    &NormalizedKey {
                        key: TerminalKey::Function(number),
                        modifiers: default(),
                    },
                    false,
                    0,
                ...
                .expect("F1 through F24 must encode")
                .is_empty()
            );
        }
        assert!(
